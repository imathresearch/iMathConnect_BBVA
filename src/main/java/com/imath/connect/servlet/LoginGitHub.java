...
public class LoginGitHub extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,IOException {
       ...
       .append("?client_id=").append(AppConfig.getProp(AppConfig.CLIENTID_GITHUB)) // the client id from the api console registration
       .append("&scope=user:email") // scope is the api permissions we are requesting
       .append("&redirect_uri=http://"+ AppConfig.getProp(AppConfig.IMATH_HOST) + ":" + AppConfig.getProp(AppConfig.IMATH_PORT) + "/iMathConnect/callbackgithub") // the servlet that google redirects to after authorization
