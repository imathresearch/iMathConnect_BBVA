...
public class LoginGoogle extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException,IOException {
		...
		System.out.println("Login google " + "http://" + Constants.IMATH_HOST() +":" + Constants.IMATH_PORT + "/iMathConnect/callbackgoogle");
	   // redirect to google for authorization
	   StringBuilder oauthUrl = new StringBuilder().append("https://accounts.google.com/o/oauth2/auth")
	   .append("?client_id=").append(Constants.CLIENTID_GOOGLE) // the client id from the api console registration
	   .append("&response_type=code")
	   .append("&scope=email") // scope is the api permissions we are requesting
	   .append("&redirect_uri=http://" + Constants.IMATH_HOST() +":" + Constants.IMATH_PORT + "/iMathConnect/callbackgoogle") // the servlet that google redirects to after authorization
